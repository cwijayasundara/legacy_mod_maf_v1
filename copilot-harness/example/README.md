...
This example shows how to use the Copilot Harness to migrate `example/lambda/handler.py`
...
# 2. Run the migration
node .copilot/scripts/migrate.js order-processor python WI-DEMO
```
...
docker run -p 8000:8000 \
  ...
  -v $(pwd)/src/lambda:/app/copilot-harness/src/lambda:ro \
  copilot-harness
